...
class MiniGPTBase(BaseModel):
    ...
    # def pc_to_cpu(self):
    #     self.pc_encoder.to("cpu")
    #     self.pc_encoder.float()

    def get_context_emb(self, prompt, pc_list):

        device = self.device

        prompt_segs = prompt.split('<PointCloudHere>')
        assert len(prompt_segs) == len(pc_list) + 1, "Unmatched numbers of   point clouds placeholders."
        seg_tokens = [
        ...
        mixed_embs = [emb for pair in zip(seg_embs[:-1], pc_list) for emb in pair] + [seg_embs[-1]]
        mixed_embs = torch.cat(mixed_embs, dim=1)
        return mixed_embs

    def prompt_wrap(self, pc_embeds, atts_pc, prompts, lengths=None):
        if prompts is None or len(prompts) == 0:
            # prompts is not provided, just return the original pc embedding
            return pc_embeds, atts_pc
        elif pc_embeds is None:
            # prompt is provided but there is no pc embedding. return the prompt embedding in right padding
            ...
            if isinstance(prompts, str):
                prompts = [prompts] * len(pc_embeds)

            for idx, (each_pc_embed, each_prompt) in enumerate(zip(pc_embeds, prompts)):
                pn = each_pc_embed.shape[-2]
                if lengths is not None:
                    each_pc_embed = each_pc_embed.reshape(-1, each_pc_embed.shape[-1])
                    each_pc_embed = each_pc_embed[:lengths[idx] * pn]
                p_segs = each_prompt.split('<PointCloudHere>')
                interleave_emb = []
                for idx, seg in enumerate(p_segs[:-1]):
                    p_tokens = self.llama_tokenizer(
                        seg, return_tensors="pt", add_special_tokens=False).to(pc_embeds.device)
                    p_embed = self.embed_tokens(p_tokens.input_ids)
                    interleave_emb.append(torch.cat([p_embed, each_pc_embed[None][:, idx * pn:(idx + 1) * pn]], dim=1))
                wrapped_emb = torch.cat(interleave_emb, dim=1)
                p_tokens = self.llama_tokenizer(
                    p_segs[-1], return_tensors="pt", add_special_tokens=False).to(pc_embeds.device)
                p_embed = self.embed_tokens(p_tokens.input_ids)
                wrapped_emb = torch.cat([wrapped_emb, p_embed], dim=1)
                emb_lists.append(wrapped_emb)

            emb_lens = [emb.shape[1] for emb in emb_lists]
            pad_emb = self.embed_tokens(torch.tensor(self.llama_tokenizer.pad_token_id, device=pc_embeds.device))

            max_length = max(emb_lens) if max(emb_lens) < self.max_context_len else self.max_context_len
            wrapped_embs = pad_emb.expand(len(emb_lens), max_length, -1).clone()
            wrapped_atts = torch.zeros([len(emb_lens), max_length], dtype=torch.int, device=pc_embeds.device)
    ...
    def preparing_embedding(self, samples):
        ...
        if 'conv_q' in samples:
            ...
            if 'length' in samples:
                # the input is a pc train (like videos)
    ...
    @torch.no_grad()
    def multi_select(self, pc, texts, answers, num_cand=None):
        all_losses = []
