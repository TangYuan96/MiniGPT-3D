...
class OpenAIOpenFreeFormClsEvaluator():
    ...
    def history_to_messages(self, history: History, system: str) -> Messages:
    ...
    def get_relpy_from_llm(self, input_sentence):
        ...
        messages.append({'role': Role.USER, 'content': query_input})
        gen = Generation.call(
    ...
    def evaluate_result(self, result):
        ...
        gpt_response, usage = self.get_relpy_from_llm(messages)
    ...
    def print_costs(self):
        print(f"Prompt Tokens Price: {self.prompt_tokens * self.price_1k_prompt_tokens / 1000:.2f} CNY")
        print(f"Completion Tokens Price: {self.completion_tokens * self.price_1k_completion_tokens / 1000:.2f} CNY")
    ...
    def save_results(self, is_temp=False):
        ...
        with open(output_path, 'w') as f:
            results_to_save = {
                ...
                'prompt_tokens': self.prompt_tokens,
                'completion_tokens': self.completion_tokens,
                'GPT_cost': f"{self.get_costs():.2f}"+" CNY",
                'results': self.response_data,
            }
            json.dump(results_to_save, f, indent=2)
